...
void first_thread_function(void const * argument)
{
  ...
  uint8_t message[50]="Thread 1 Ola, Alligator\n\r";
  ...
  for(;;)
  {
   ...
   HAL_UART_Transmit(&huart3, message, 60, 10);
...
void second_thread_function(void const * argument)
{
  ...
  /* Infinite loop */
	...
	uint8_t message[50]="Thread 2 Ola, Alligator\n\r";
  ...
  for(;;)
  {
	...
	{
		//TODO Notify thread 3
		xTaskNotify(thirdThreadHandle,0x01,eSetBits);
...
void third_thread_function(void const * argument)
{
	...
	uint8_t message[50]="Thread 3 Ola, Alligator\n\r";
	uint32_t notifyValue;

	for(;;)
	{
			//TODO Wait for the notification to arrive from Thread 2
			xTaskNotifyWait(pdFALSE, 0xFF, &notifyValue, portMAX_DELAY); //pdFALSE -> DO NOT RESET FLAGS WHEN ENTRYING, 0xFF -> Reset flags when exit
